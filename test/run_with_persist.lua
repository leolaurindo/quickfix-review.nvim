vim.env.XDG_DATA_HOME = "/tmp/quickfix-notes-persist-test-data"
vim.env.XDG_STATE_HOME = "/tmp/quickfix-notes-persist-test-state"
...
assert(persist.load({ namespace = "quickfix-notes", name = "integration", scope = scope }))
assert(persist.delete({ namespace = "quickfix-notes", name = "integration", scope = scope }))

print("quickfix_notes with persist tests passed")
