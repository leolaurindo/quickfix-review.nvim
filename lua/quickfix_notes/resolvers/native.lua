...
function M.location(bufnr, winid)
	return require("quickfix_notes.resolvers.normal").location(bufnr, winid)
end

function M.range_location(bufnr, start_line, stop_line)
	return require("quickfix_notes.resolvers.normal").range_location(bufnr, start_line, stop_line)
end
