...
available. Response generation is optional; when the user explicitly requests
the Quickfix Review skill, it returns a version 2 notes payload through the
...
warns on failure. Review imports through the normal merge path and deletes both
files after success. Their disappearance confirms successful consumption, not
failure. The mailbox is not persistent or synchronized state. Version 1 findings
payloads remain accepted for compatibility.
